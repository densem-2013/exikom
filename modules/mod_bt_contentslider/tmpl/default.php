		...
		<?php foreach( $list as $i => $row ): ?>
			<div class="bt-row <?php if($i==0) echo 'bt-row-first'; else if($i==count($list)-1) echo 'bt-row-last' ?>"  style="width:<?php echo $itemWidth;?>%" >
				...
				<?php if($show_category_name_as_link) : ?>
					...
					<?php if( $showTitle ): ?>
					<h3 class="jsn-moduletitle"><span class="jsn-moduleicon">
					<a class="bt-title" target="<?php echo $openTarget; ?>"
						title="<?php echo $row->title; ?>"
						href="<?php echo $row->link;?>"> <?php echo $row->title_cut; ?> </a></span></h3>
					<?php endif; ?>
					<?php if( $row->thumbnail && $align_image == "center" ): ?>
					<div class="bt-center">
					...
					<?php if( $showAuthor ): ?>
						...
						<?php endif; ?>
						<?php if( $showDate ): ?>
						<span class="bt-date"><?php echo JText::sprintf('BT_CREATEDON',date('d/m/Y', strtotime($row->date))); ?>
						</span>
						<?php endif; ?>
					</div>
					<?php endif; ?>
					...
					<?php if( $showReadmore && ($row->fulltext)) : ?>
					<p class="readmore">
						<a class="link-action" target="<?php echo $openTarget; ?>"
							title="<?php echo $row->title;?>"
							href="<?php echo $row->link;?>"><!--<span class="icon-chevron-right"></span> --><?php echo JText::_('READ_MORE');?>
						</a>
					</p>
					<?php endif; ?>
