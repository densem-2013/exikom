void function ( exports, $, _, Backbone ) {
	...
	exports.ES_Item_Style = B.Model({
		...
		box_shadows: B.Collection(
			B.Model({ inset: false, x: 0, y: 0, blur: 0, color: '' })
		),
	...
	exports.ES_Item = B.Model({
		...
		updateOffset: function() {
			...
			switch (parseFloat((pos_x))) {
				case 0:   this.set('style.offset.x', 0); break;
				case 0.5: this.set('style.offset.x', parseInt(this.get('style.width')) / -2); break;
				case 1:   this.set('style.offset.x', -this.get('style.width')); break;
			}
			var pos_y =this.get('style.position.y');
			switch (parseFloat((pos_y))) {
				case 0:   this.set('style.offset.y', 0); break;
				case 0.5: this.set('style.offset.y', this.get('style.height') / -2); break;
				case 1:   this.set('style.offset.y', -this.get('style.height')); break;
			}
		}
